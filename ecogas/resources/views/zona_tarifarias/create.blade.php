...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-plus"></i> ZonaTarifarias / Create </h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('zona_tarifarias.store') }}" method="POST">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('zona')) has-error @endif">
                       <label for="zona-field">Zona</label>
                    <input type="text" id="zona-field" name="zona" class="form-control" value="{{ old("zona") }}"/>
                       @if($errors->has("zona"))
